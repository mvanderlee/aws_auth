...
@click.option(
    ...
    envvar="MFA_ARN",
    default=None,
    help='The identification number of the MFA device that is associated with the IAM user. i.e.: "arn:aws:iam::123456789012:mfa/tony.stark". You can find this on the IAM page.',
)
...
@click.option(
    ...
    "--source-profile",
    default="mfa-source",
    envvar="SOURCE_PROFILE",
    help="What AWS profile to get the session token with.",
)
@click.option(
    "-tp", "--target-profile", default="default", envvar="TARGET_PROFILE", help="What AWS profile to store the credentials under."
)
...
@click.option(
    ...
    "--target-profile",
    default="default",
    envvar="TARGET_PROFILE",
    help="What AWS profile to store the credentials under.",
)
@click.option("-v", "--verbose", default=False)
def export(
    source_profile,
...
def auto_detect_mfa_device(session: boto3.Session) -> str:
    ...
    if len(mfa_devices) == 0:
        ...
        logger.warning(
            f"No MFA device was specified and multiple MFA device were detected in IAM. Will use the first one: {mfa_arn}"
        )
    else:
        mfa_arn = mfa_devices[0]["SerialNumber"]
